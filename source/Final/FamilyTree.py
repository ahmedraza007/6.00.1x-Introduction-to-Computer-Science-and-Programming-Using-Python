...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        ancestor = self.founder
        
        dist_a = self.get_distance(ancestor, a_node)
        dist_b = self.get_distance(ancestor, b_node)
        print dist_a, dist_b
        diff = (dist_a - dist_b)
        
        if (str(a_node) == str(b_node)) or self.direct_descendant(a_node, b_node, diff):
            return -1, diff
#         elif diff == -1:
#             cousin_type = min(abs(dist_a), abs(dist_b))
#             return cousin_type, abs(diff)
        else:
            cousin_type = min(dist_a, dist_b) - 1
            return cousin_type, abs(diff)
        
        
    def direct_descendant(self, node1, node2, diff):
        for i in range(diff):
            if str(node1) == str(node2.get_parent()):
                return True
            else:
                continue
        
        for i in range(diff):
            if str(node1.get_parent()) == str(node2):
                return True
            else:
                continue
        return False
...
t, r = f.cousin("e", "i")
print "'e' is a", words[t], "cousin", r, "removed from 'i'"

t, r = f.cousin("a", "m")
print "'a' is a", words[t], "cousin", r, "removed from 'm'"

t, r = f.cousin("k", "j")
print "'k' is a", words[t], "cousin", r, "removed from 'j'"


t, r = f.cousin("q", "o")
print "'q' is a", words[t], "cousin", r, "removed from 'o'"